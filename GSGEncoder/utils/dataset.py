...
from load_data import my_read_tu_data,msy_read_tu_data
...
class MsyBlockChain_TUDataset(InMemoryDataset):
    r"""A variety of graph kernel benchmark datasets, *.e.g.* "IMDB-BINARY",
    "REDDIT-BINARY" or "PROTEINS", collected from the `TU Dortmund University
    <https://chrsmrrs.github.io/datasets>`_.
    In addition, this dataset wrapper provides `cleaned dataset versions
    ...
    def __init__(self, root: str, name: str,x,edge_index,
                 transform: Optional[Callable] = None,
                 pre_transform: Optional[Callable] = None,
                 pre_filter: Optional[Callable] = None,
                 use_node_attr: bool = False, use_edge_attr: bool = False, use_node_importance=False,
    ...
    def process(self):
        self.data, self.slices = msy_read_tu_data(self.raw_dir, self.name, self.x, self.edge_index)
