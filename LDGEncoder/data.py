...
# data process and graph construct
...
def construct_graph(rows, batch_num,max_link, max_n,features):
    # Save all nodes, labels and features of positive and negative samples
    Nodes = []
    Labels = []
    # features = []
    ...
    # Record the generated negative examples
    # Neighbor matrix-> A
    # all nodes label-> L
    # all nodes feature-> F
    A = []
    ...
    # Build the graph for each small node, and each small node is divided into ten parts
    for node in Nodes:
        print(node)
        if num == 0:
            A.append([])
        ...
        # Record the number of all nonempty graphs
    ...
    # i-th batch
    for i in range(len(Adj)):
        # The j-th node within each batch
        for j in range(batch_num):
            # Each time slice of the j-th node
